	                	...
	                	@foreach($posts as $post)
	                		<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
		                		<span class="chatter_posts">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id || Auth::user()->is_admin()))
			                		...
			                		<div class="chatter_avatar">
					        			...
					        			@else
					        				<span class="chatter_avatar_circle">
					        					<img class= "comment-profile" src="/uploads/avatars/{{ $post->user->avatar }}">
					        		...
					        		<div class="chatter_middle">
					        			<span class="chatter_middle_details"><a href=" {{ '/user/'.$post->user_id }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
